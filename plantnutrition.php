...
<html>
	<head>
		...
		.dropdown a:hover {background-color: #2CCE00;}
	...
	<!--<div>
		<ul class="menu-bar">
			<li class="lfloat"><a href="index.php" class="menu-bar-list">HOME</a></li>
			...
			<li class="lfloat"><a href="plantprotection.php" class="menu-bar-list">PLANT PROTECTION</a></li>
			<li class="lfloat"><a href="plantnutrition.php" class="active menu-bar-list">PLANT NUTRITION</a></li>
			<li class="lfloat"><a href="machinery.php" class="menu-bar-list">MACHINERY</a></li>
		</ul>
	</div>-->
			...
			<div class="card" style="margin-top:10px;margin-left:-20px;width:100%;">
				...
				<div class="check-list">
						<h3 style="color:#5B6775" >Category</h3>
							...
							$query = "SELECT DISTINCT(category) FROM product WHERE type='Plant Nutrition'";
...
<!--				</div>
				...
				<br>
					<h3 style="color:#5B6775" >Brand</h3>
						<div style="max-height:300px; overflow-y: auto; overflow-x: hidden;">
						<?php
						$query = "SELECT DISTINCT(brand) FROM product WHERE type='Plant Nutrition'";
						$result = mysqli_query($connect,$query);
				...
				<div class="check-list">
					...
					<h3 style="color:#5B6775" >State</h3>
						<div style="max-height:300px; overflow-y: auto; overflow-x: hidden;">
						<?php
						$query = "SELECT DISTINCT(state) FROM product INNER JOIN shop ON product.shop_id=shop.shop_id WHERE type='Plant Nutrition'";
						$result = mysqli_query($connect,$query);
...
<!--						<button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown">State
				...
				<div class="check-list">
					...
					<h3 style="color:#5B6775" >District</h3>
						...
						<?php
						$query = "SELECT DISTINCT(district) FROM product INNER JOIN shop ON product.shop_id=shop.shop_id WHERE type='Plant Nutrition'";
						$result = mysqli_query($connect,$query);
...
$(document).ready(function(){
    ...
    function filter_data()
    {
        ...
        $.ajax({
            url:"fetch_data.php",
            method:"POST",
            data:{action:action, minimum_price:minimum_price, maximum_price:maximum_price,type:type,category:category,brand:brand,state:state,district:district},
...
/*		$("#hightolow").click(function(){  
           ...
           $.ajax({  
                url:"fetch_data.php",  
                method:"POST",  
                data:{action:action, minimum_price:minimum_price, maximum_price:maximum_price,type:type,category:category,brand:brand,state:state,district:district;hightolow:hightolow},
                success:function(data){  
                     $('.filter_data').html(data);  
                }  
           });  
